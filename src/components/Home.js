import React, {Component} from 'react'
import {BrowserRouter} from 'react-router-dom'
import Navigation from './Navigation'
import ArrayCards from './ArrayCards'
// import Footer from './Footer'
// import SearchActivity from './SearchActivity'
// import Scroll from './Scroll'

const dataActivity = [
  {
  id: '5c993d15d4958e3eb3654d2c',
  name: 'Migriño ATV Tour (Single ATV)',
  image: 'https://www.cactusatvtours.com/assets/images/tour-landing/mbdt/header-cover.jpg',
  taxonomy: ['out-doors'],
  description: 'Enjoy the beauty of nature driving through the desert, mountains, canyons a huge dry stream and beautiful beach with sand dunes and spectacular ocean side cliffs. This is the best ATV tour around the world. Don´t miss it.',
  company: 'cactus-atv-tours',
  price: {
    adults: 110
  },
  duration: 150,
  restrictions: [],
  included: [
    'Round trip transportation',
    'Water',
    "Helmet",
    "Goggles",
    "Bandana"
  ],
  schedules: [800, 1000, 1330, 1600]
},
{
  id: '5c993dbfd4958e3eb3654d2d',
  name: 'Migriño ATV Tour (Double ATV)',
  image: 'https://cactusatvtours.com/assets/images/tour-landing/mbdt/1.jpg',
  taxonomy: ['out-doors'],
  description: 'Enjoy the beauty of nature driving through the desert, mountains, canyons a huge dry stream and beautiful beach with sand dunes and spectacular ocean side cliffs. This is the best ATV tour around the world. Don´t miss it.',
  company: 'cactus-atv-tours',
  price: {
    adults: 130
  },
  duration: 150,
  restrictions: [],
  included: [
    "Round trip transportation",
    "Water",
    "Helmet",
    "Goggles",
    "Bandana"
  ],
  schedules: [800, 1000, 1330, 1600]
},
{
  id: '5c993e06d4958e3eb3654d2e',
  name: 'Side by Side Sport Adventure (Razors)',
  image: 'https://cactusatvtours.com/assets/images/tour-landing/bda/1.jpg',
  taxonomy: ['out-doors'],
  description: 'Be prepared for the highlight of your vacation, this off road adventure is focused on making you have the fun of your life. You will ride on desert trails that are a wild treat offering a new world to explore and you will have an exciting ride on two beautiful white sand beaches with huge dunes and beautiful views. FIRST TIMERS: don’t worry no experience necessary. If you know how to drive a regular automatic car, you will be fine.',
  company: 'cactus-atv-tours',
  price: {
    adults: 205,
    kids: 58
  },
  duration: 150,
  restrictions: [],
  included: [
    'Round trip transportation',
    'Water',
    "Helmet",
    "Goggles",
    "Bandana"
  ],
  schedules: [800, 1000, 1330, 1600]
},
{
  id: '5c993e1dd4958e3eb3654d2f',
  name: 'Zip-Line',
  image: 'http://www.costaazulziplines.com/images/caz-2%20(large)-crop-u1270.jpg',
  taxonomy: ['out-doors'],
  description: 'Easy: Its a very basic principle, you ride on an aircraft grade steel cable using a pulley and a safety harness with a main safety strap wich can carry over 20 times your own weight and a second one acting as a backup...',
  company: 'canopy-costa-azul',
  price: {
    adults: 84.0,
    kids: 58.0
  },
  duration: 0,
  restrictions: [
    'Kids from 7 to 12 years old.',
    'Please contact us in case of cancellation at least 24 hours before your tour time, otherwise it wont be reimbursable.'
  ],
  included: [],
  schedules: []
},
{
  id: '5c993e35d4958e3eb3654d30',
  name: 'Camel Ride',
  image: 'https://www.cactusatvtours.com/assets/images/tour-landing/cml/header-cover.jpg',
  taxonomy: ['out-doors'],
  description: 'What is the most unique & unusual thing you can do in Cabo? A camel ride on the beach of course! Don’t miss the fantastic opportunity to experience an incredible tour that you will remember for a life time. Come and enjoy a spectacular camel ride along the crystalline waters and majestic waves of the Pacific Ocean. Immerse yourself in camel culture and learn about its history, anatomy, personality, keeping and training; Get up close and personal including petting and feeding these amazing creatures. Taste our exquisit mexican food and discover the authentic flavors of several dishes from different regions of our country.',
  company: 'cactus-atv-tours',
  price: {
    adults: 95
  },
  duration: 150,
  restrictions: [],
  included: ['Round trip transportation', 'Water', 'Helmet', 'Buffet'],
  schedules: [900, 1200, 1500]
},
{
  id: '5c993e4ed4958e3eb3654d31',
  name: 'Cabo Snorkel & Sea Adventure',
  image: 'https://cdn.vallarta-adventures.com/sites/default/files/2018-02/snorkel-sea-adventure-cabo-adventures-8_0.jpg',
  taxonomy: ['sea-adventure'],
  description: 'Slip on your snorkel and jump in for an unforgettable ocean expedition. Cruise along the picturesque coastline and listen to stories about the impressive attractions at Land’s End like the Arch, Lovers Beach, the sea lion colony and admire the fascinating rock formations and landmarks that define Cabo San Lucas. This is snorkeling in Cabo, and you’re invited...',
  company: 'cabo-adventure',
  price: {
    adults: 89,
    kids: 69
  },
  duration: 180,
  restrictions: ['Min 5 years Old', 'Maximum weight: 265 lbs (120 kg)'],
  included: [
    "Water",
    "Snacks",
    "Towels",
    "Equipment (snorkel, mask, fins)",
    'SUP (Stand up paddle board)',
    'Kayak',
    "Sea Scooter",
    "Wetsuits",
    'Round trip transportation in an air-conditioned van from most hotels in Cabo San Lucas and San José del Cabo'
  ],
  schedules: [900, 1000, 1400]
},
{
  id: '5c993e78d4958e3eb3654d33',
  name: 'Cabo Zip Line Outdoor Adventure',
  image: 'https://cdn.vallarta-adventures.com/sites/default/files/2018-02/outdoor-adventure-cabo-adventures-8_0.jpg',
  taxonomy: ['out-doors'],
  description: 'Some people come to Cabo to relax. Others come to adventure. If you’re looking to add a little (or a lot) of adventure to your Cabo stay, there is no better choice than our Outdoor Zip Line Adventure tour! Join our expert guides for a Cabo zip line tour, tropical hike, and fun Polaris UTV off-roading excursions. Feel the rush of the best Cabo San Lucas zip line courses available, and take in some of the region’s most breathtaking views along the way. If adventure is your style, this is the tour for you....',
  company: 'cabo-adventure',
  price: {
    adults: 129,
    kids: 99
  },
  duration: 210,
  restrictions: [
    'Min 8 years Old',
    'Maximum weight: 260 lbs (118Kg)',
    "Child prices apply to children ages 8 to 11. Any child age 12 years and older is considered an adult"
  ],
  included: [
    'Professional gear',
    "Bandana",
    'Bottled water',
    "Snack",
    'Round trip transportation in an air-conditioned van from most hotels in Cabo San Lucas and San José del Cabo'
  ],
  schedules: [715]
},
{
  id: '5c993e8ed4958e3eb3654d34',
  name: 'Dolphin Signature Swim',
  image: 'https://cdn.vallarta-adventures.com/sites/default/files/2018-02/dolphin-swim-experience-vallarta-adventures-4_0.jpg',
  taxonomy: ['sea-adventure'],
  description: 'Experience the fun and excitement of Swimming with Dolphins! Feel the excitement wash over you as you wade into the refreshing dolphin pool and get nose-to-bottlenose with our friendly family of dolphins. The Dolphin Signature Swim is the most fun and informative program available anywhere. We offer small group interactions for this 40-minute encounter so that you are ensured the best opportunity to create a bond with your new flippered friend. Accompanied by our Marine Mammal Expert, you will learn all about the Pacific Bottlenose Dolphins in the comfort and safety of our crystal-clear saltwater pool, specially designed for this type of program.',
  company: 'cabo-adventure',
  price: {
    adults: 189,
    kids: 99
  },
  duration: 40,
  restrictions: [
    "Min 4 years Old",
    'Cameras allowed in observation areas only',
    "No jewelry or sunscreen allowed",
    "Lifejackets are provided and required"
  ],
  included: [
    "Towel",
    "Bottled water",
    "Lifejacket",
    "Locker",
    "Wetsuit",
    "Round trip transportation in an air-conditioned van from most hotels in Cabo San Lucas and San José del Cabo"
  ],
  schedules: [900, 1000, 1100]
},
{
  id: '5c993ea0d4958e3eb3654d35',
  name: 'Cabo ATV Tour Off-Road Adventure',
  image: 'https://cdn.vallarta-adventures.com/sites/default/files/2018-03/off-road-adventure-cabo-adventures-10.jpg',
  taxonomy: ['out-doors'],
  description: 'Ready for an off-road adventure you won’t soon forget? Our Cabo San Lucas ATV is the perfect choice for you. This adrenaline-pumping ATV Cabo tour takes you through the spectacular landscape surrounding Cabo, giving you the chance to experience it like never before. Explore the Baja outback aboard a powerful four-seater Polaris ATV and learn more about the region with information from our expert guides. Our all-terrain vehicles are safe and easy to drive, great for riders young and old. Take your Cabo adventure to the next level and go off-roading on our award-winning Cabo San Lucas ATV tour here at Cabo Adventures!',
  company: 'cabo-adventure',
  price: {
    adults: 299
  },
  duration: 180,
  restrictions: [
    "Min 5 years Old",
    "Maximum weight: 265 lbs / 120 kgs",
    "Minimum height: 4 ft /1.20 m",
    "A $30 USD per vehicle insurance fee for collision / damage will be charged at check-in",
    'For safety reasons persons with back or neck, heart, balance or/and dizziness conditions are restricted from this tour',
    'For safety reasons, the use of cameras and GoPros is not permitted'
  ],
  included: [
    "Snack, Bandana",
    'Beverages',
    'Round trip transportation in an air-conditioned van from most hotels in Cabo San Lucas and San José del Cabo'
  ],
  schedules: [1400]
},
{
  id: '5ca16f5cc387c1061d9d432b',
  name: 'Snorkel Fun with Lunch',
  image: 'http://caboescapetours.com/wp-content/uploads/2014/11/Snk-Fun01.jpg',
  taxonomy: ['sea-adventure'],
  description: 'Visit Cabo’s world-famous Arch at Land’s End, followed by a scenic cruise up the Sea of Cortez, known all over the world for its spectacular sea life. Enjoy a freshly prepared lunch on board, and party all the way back to the dock with an open bar, DJ, and fun activities.',
  company: 'Cabo Escape',
  price: {
    adults: 79,
    kids: 39.5
  },
  duration: 180,
  restrictions: [],
  included: ['Lunch'],
  schedules: [1230]
},
{
  id: '5ca17f5ac387c1061d9d4376',
  name: 'Sunset Fajita Cruise',
  image: 'http://caboescapetours.com/wp-content/uploads/2014/11/Sunset03.jpg',
  taxonomy: ['sea-adventure'],
  description: 'No trip to Cabo is complete without a sunset cruise! Come aboard the Cabo Escape for a memorable cruise to the world-famous Land’s End, where you can see amazing rock formations and the popular Arch. Watch from an open-air upper deck as the sun disappears into the Pacific Ocean, creating a breathtakingly colorful sunset! Dance the night away on the top deck with our crew or just relax and enjoy the music below. There’s fun for everyone! ',
  company: 'Cabo Escape',
  price: {
    adults: 89,
    kids: 49.5
  },
  duration: 120,
  restrictions: [],
  included: ['Lunch'],
  schedules: [1700]
},
{
  id: '5ca1818bc387c1061d9d43a5',
  name: 'Yo Ho Ho Sunset Cruise (Cabo Legend)',
  image: 'https://fh-sites.imgix.net/sites/1903/2018/10/23203650/Yo-Ho-Ho-Sunset-Cruise-image-1.jpg?auto=compress%2Cformat&fit=crop&crop=faces&w=720&h=480',
  taxonomy: ['sea-adventure'],
  description: 'Hop aboard our wooden pirate ship and hoist sails as part of our hands-on pirate adventure. It’s going to be a wild pirate party! Hear tall tales of Cabo’s pirate history and secrets. Enjoy the wind in your hair and the famous Baja sunsets as you feast on mouthwatering BBQ ribs and chicken prepared with our galley secret pirate recipe (we pirates hate canned products) plus wine, beer, and tasty cocktails from our open bar. We recommend the grog!',
  company: 'Cabo Legend',
  price: {
    adults: 99,
    kids: 49.5
  },
  duration: 145,
  restrictions: [],
  included: ['Lunch'],
  schedules: [1745]
},
{
  id: '5ca183c0c387c1061d9d43d2',
  name: 'Treasure Hunt Lunch Snorkel',
  image: 'https://fh-sites.imgix.net/sites/1903/2018/10/23230204/MG_8680.jpg',
  taxonomy: ['sea-adventure'],
  description: 'Cabo has been home to pirates since the 1500s. Cruise back in time aboard our 100-foot wooden early-1900s pirate sailboat, and see the world-famous arch while hearing tall tales about Cabo history and the pirates that sailed its seas. Strike our real, working sails and drop anchor to get quality time with the tropical fish.',
  company: 'Cabo Legend',
  price: {
    adults: 79,
    kids: 39.5
  },
  duration: 180,
  restrictions: [],
  included: [
    "chicken burritos",
    "guacamole",
    "chips",
    "salads",
    "yummy beef"
  ],
  schedules: [1200]
},
{
  id: '5ca184b8c387c1061d9d43e5',
  name: 'Whale Watching & Breakfast in Cabo',
  image: 'http://caboescapetours.com/wp-content/uploads/2014/11/whales02.jpg',
  taxonomy: ['sea-adventure'],
  description: 'Get close to nature and see Cabo’s whales aboard our twin-level power catamaran.Your cruise begins with a scenic view of Cabo’s famous Arch and beautiful coastline. Get beautiful photos to share the memories of your vacation. Cabo Escape adheres to Whale Watching Guidelines and practices respectful wildlife viewing. Our knowledgeable marine naturalists are there to inform and amuse you. Our crew loves their work and it shows. Guests tell us that the staff is attentive, warm and friendly.',
  company: 'Cabo Escape',
  price: {
    adults: 69,
    kids: 34.5
  },
  duration: 120,
  restrictions: [],
  included: ['breakfast', 'coffee', 'waffles', 'the best mimosas'],
  schedules: [900]
}
]




class Home extends Component {
  constructor(){
    super()
    this.state = {
      activities: [],
      input: ""
    }
  }

  componentDidMount(){
    this.setState({
      activities: dataActivity
    })
  }

  onInputChange = (e) => {
    this.setState({input:e.target.value})
  }

  render(){

    const filterActivities = this.state.activities.filter((activity) => {
      return activity.name.toLowerCase().includes(this.state.input.toLowerCase())
    })

    return (
      
        <div>
          {/* <SearchActivity onInputChange={this.onInputChange}/> */}
          {/* <Scroll> */}
          <p>Activities</p>
            <ArrayCards dataActivity={filterActivities} />
          {/* </Scroll>
          <Footer/> */}
        </div>
      
);
  }
  
}

export default Home;